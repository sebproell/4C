...
namespace
{
  ...
  template <int dim, int spacedim>
  void output_triangulation(
      dealii::DoFHandler<dim, spacedim>& dof_handler, const std::string& file_name, bool mixed)
  {
    if (do_output)
    {
      dealii::DataOut<dim, spacedim> data_out;
      ...
      if (mixed)
      {
        const bool first_is_hex =
            dof_handler.get_fe(0).reference_cell() == dealii::ReferenceCells::Hexahedron;
        const dealii::hp::MappingCollection<dim> mapping(
            first_is_hex ? dealii::MappingFE<dim>(dealii::FE_Q<dim>(1))
                         : dealii::MappingFE<dim>(dealii::FE_SimplexP<dim>(1)),
            first_is_hex ? dealii::MappingFE<dim>(dealii::FE_SimplexP<dim>(1))
                         : dealii::MappingFE<dim>(dealii::FE_Q<dim>(1)));
