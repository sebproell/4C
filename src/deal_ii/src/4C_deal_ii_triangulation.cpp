...
namespace DealiiWrappers
{
      ...
      dealii::Triangulation<dim, spacedim>& tria, const Core::FE::Discretization& discretization)
  {
    ...
    // copy the node coordinates and gids
    {
      ...
      AssertDimension(all_coarse_cell_vertices.size(), all_node_gids.size());
      ...
      AssertDimension(communicated_vertices, discretization.num_global_nodes());
...
#ifdef DEBUG
    {
      ...
      AssertDimension(communicated_cells, discretization.num_global_elements());

      const auto communicated_gids = std::accumulate(all_element_gids.begin(),
          all_element_gids.end(), 0, [](int n, auto& v) { return n + v.size(); });
      AssertDimension(communicated_gids, discretization.num_global_elements());
      ...
      AssertDimension(communicated_centers, discretization.num_global_elements());
    }
#endif
    ...
    // Note: we can only create the Context that maps between the two discretizations when using pfT
    {
                       ...
                       &tria))
      {
        ...
        for (const auto& cell : tria.active_cell_iterators())
        {
          ...
          Assert(found != my_element_centers.end(), dealii::ExcInternalError());
        ...
        Internal::FillMapping(context, discretization);
      }
      else
      {
    ...
    AssertDimension(tria.n_global_coarse_cells(), discretization.num_global_elements());
