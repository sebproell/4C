    ...
    Core::IO::ValueParser& parser, Core::IO::InputParameterContainer& container) const
{
  ...
  parse_in_arbitrary_order(parser, data.entries, subcontainer);
    ...
    Core::IO::ValueParser& parser, Core::IO::InputParameterContainer& container) const
{
  ...
  auto component = data.entries.begin();
  for (; component != data.entries.end(); ++component)
  {
    ...
    if (success)
    {
      ...
      for (auto other = data.entries.begin(); other != data.entries.end(); ++other)
      {
...
Core::IO::InputSpec Core::IO::InputSpecBuilders::group(
    std::string name, Core::IO::InputSpecBuilders::GroupData data)
{
  FOUR_C_ASSERT_ALWAYS(!data.entries.empty(), "A group must contain at least one entry.");

  assert_unique_or_empty_names(data.entries);

  InputSpec::CommonData common_data{
      .name = name,
      .description = data.description,
      .required = data.required,
      .has_default_value = all_have_default_values(data.entries),
  };

  return InputSpec(Internal::GroupSpec{.name = name, .data = std::move(data)}, common_data);
}
...
Core::IO::InputSpec Core::IO::InputSpecBuilders::group(std::vector<InputSpec> specs)
{
  ...
  return InputSpec(
      Internal::GroupSpec{.name = "",
          .data = {.description = description, .required = true, .entries = std::move(specs)}},
      common_data);
}
...
Core::IO::InputSpec Core::IO::InputSpecBuilders::one_of(std::vector<InputSpec> specs)
{
  ...
  GroupData group_data{
      .description = description,
      .required = true,
      .entries = std::move(specs),
  };

  return InputSpec(Internal::OneOfSpec{.data = std::move(group_data)}, std::move(common_data));
}
