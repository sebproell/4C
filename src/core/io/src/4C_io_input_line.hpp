...
namespace Core::IO
{
  ...
  struct Component
  {
   ...
   public:
    ...
    {
      component_->parse(parser, container);
    }
    ...
    {
      if (!required()) stream << "[";
      component_->print(stream, container);
      if (!required()) stream << "]";
      stream << " ";
    }
  ...
  namespace InputLineBuilders
  {
     ...
     * any type. The remaining parameterization options follow the same rules as for the entry()
     * function.
     */
    template <typename T>
    ...
    /**
     * Exactly one of the given components is expected. There is no support for default values; one
     * of the components must fit the input.
     *
     * @note The components are tried in the order they are given. The first component that fits the
     * input is used. If multiple components fit or none of the components fits, an exception is
     * thrown.
     */
    Component one_of(std::vector<Component> components);
  }  // namespace InputLineBuilders
...
template <typename DataType>
void Core::IO::InputLineBuilders::Internal::BasicComponent<DataType>::parse(
