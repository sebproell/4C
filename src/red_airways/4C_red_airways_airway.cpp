    ...
    std::map<std::string, std::map<std::string, Core::IO::InputSpec>>& definitions)
{
  ...
  defs["LINE2"] = all_of({
      ...
      parameter<double>("AirwayColl", {.required = false}),
      parameter<double>("S_Close", {.required = false}),
      parameter<double>("S_Open", {.required = false}),
      parameter<double>("Pcrit_Open", {.required = false}),
      parameter<double>("Pcrit_Close", {.required = false}),
      parameter<double>("Open_Init", {.required = false}),
      parameter<double>("BranchLength", {.required = false}),
  });
}
